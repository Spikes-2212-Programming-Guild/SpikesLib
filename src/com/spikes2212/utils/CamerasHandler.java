...
public class CamerasHandler {
	...
	/**
	 * Constructs a new instance of {@link CamerasHandler} that automatically starts
	 * streaming to the <a href=
	 ...
	 */
	public CamerasHandler(int width, int height, int... ports) throws IndexOutOfBoundsException {
		for (int i : ports) {
	...
	/**
	 * Switches the stream to the camera with the given port. If there's no camera
	...
	/**
	 * Sets the exposure of the camera in the given port. If there's no camera with
	 * that port in the {@link CamerasHandler}, does nothing.
	 *
	 * @param port
	 ...
	 * If a camera is added after running {@link #setExposure(int)}, it will have
	 * the default automatic exposure, meaning you will have to run
	 * {@link #setExposure(int, int)} again if you want it to have none-automatic
	 * exposure.
	 * 
	 * @param exposure
